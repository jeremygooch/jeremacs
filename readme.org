...
#+BEGIN_SRC emacs-lisp
  ...
  (setq exec-path (append exec-path '("/Users/jeremygooch/.sdkman/candidates/java/11.0.16-zulu/bin")))
  (setq exec-path (append exec-path '("/Users/jeremygooch/.sdkman/candidates/groovy/4.0.5/bin")))
...
#+BEGIN_SRC emacs-lisp
  ...
  ;; (setenv "PATH" "/Users/jeremygooch/.sdkman/candidates/java/11.0.16-zulu/bin")
  ;; (setenv "PATH" "/Users/jeremygooch/.sdkman/candidates/groovy/4.0.5/bin")
  (setenv "JAVA_HOME" "/Library/Java/JavaVirtualMachines/jdk-14.0.2.jdk/Contents/Home")
...
#+END_SRC
**** Spellcheck
Use aspell for Mac. =aspell= can be installed with mac ports:
- =sudo port install aspell=
- =sudo port install aspell-dict-en= (for installing the english dictionary)
#+BEGIN_SRC emacs-lisp :tangle (if (string-equal system-type "darwin") "~/.emacs.d/init.el" "no")
...
Adjust the automatic scrolling
#+begin_src emacs-lisp
(setq scroll-conservatively 10
      scroll-margin 25
      maximum-scroll-margin .3)
#+end_src
...
#+end_src
**** Window Splitting
For larger displays and such the default split goes horizontal when I'd prefer vertical. After some searching I found this solution from: https://lists.gnu.org/archive/html/help-gnu-emacs/2015-08/msg00339.html
...
Rename file and register with version control by default
#+begin_src emacs-lisp
...
Org Modern provides solid prettifying of org buffers
#+BEGIN_SRC emacs-lisp
  ...
  (use-package org-modern
    ...
    (setq org-modern-fold-stars
	  '(("▶" . "⏷")
	    ("▷" . "▽") ;; this was borked on the initial install, so just copy the previous level's value
	    ("⏵" . "⏷")
	    ("▸" . "▾")
	    ("▹" . "▿")
	    ))
    :config
    ;; (custom-set-faces '(org-modern-date-active ((t (:extend t :background "unspecified" :foreground "unspecified")))))
...
#+begin_src emacs-lisp
    (custom-set-faces
     ...
     '(org-block-begin-line ((t (:extend t :background "#191d27" :foreground "#617776" :height 0.9))))
...
*** Swapping windows within a frame
I often end up needing to quickly swap buffers in a layout so here's a simple elisp to save on keystrokes.
#+begin_src emacs-lisp
(defun swap-windows ()
  ...
  (let* ((this-window (selected-window))
    ...
    (unless (eq this-window next-window)
      (set-window-buffer this-window next-buffer)
...
#+end_src
**** Org TOC
I frequently add org files as GH readmes or gists where having a TOC is helpful. org-toc helps with that:
#+begin_src emacs-lisp
(use-package toc-org
  :config (if (require 'toc-org nil t)
	      (progn (add-hook 'org-mode-hook 'toc-org-mode))))
#+end_src
This will generate a new TOC on save for any headline with a relevant :toc: tag. More info: https://github.com/snosov1/toc-org?tab=readme-ov-file#use
*** Misc
Always focus on a help buffer just opened so you don't have to navigate to it.
#+begin_src emacs-lisp
...
Fix annoying ui features
#+begin_src emacs-lisp
  ;; turn off problematic modes
...
#+begin_src emacs-lisp
  ...
  (delete '("\\.js\\'" . javascript-mode) auto-mode-alist)

  ;;(add-to-list 'auto-mode-alist '("\\.js[mx]?\\'" . js2-mode))
  (add-to-list 'auto-mode-alist '("\\.js\\'" . js2-mode))
#+end_src

Override the default =M-.= keybinding from lsp-find-definition to use the lsp-ui version for js2 mode. Seems to work better for js only projects.
#+begin_src emacs-lisp
...
***** LSP Booster
#+begin_src emacs-lisp :tangle (if (string-equal system-type "darwin") "~/.emacs.d/init.el" "no")
  ;; LSP booster on mac has to be compiled from source due to lack of proper code signing: https://github.com/blahgeek/emacs-lsp-booster?tab=readme-ov-file#obtain-or-build-emacs-lsp-booster
  ;; Point to the binary build from src instead
  (add-to-list 'exec-path "/Users/jeremygooch/src/emacs-lsp-booster/target/release/")
#+end_src
...
Adding prettier-js. Be sure prettier is installed with =npm i -g prettier=.
#+begin_src emacs-lisp
...
#+begin_src emacs-lisp
  (use-package dape
    ...
    ;;  To not display info/buffers on startup
...
#+begin_src emacs-lisp
  (use-package go-mode
    :custom
    (gofmt-command "/Users/jeremygooch/go/bin/goimports")
    :config
...
#+BEGIN_SRC emacs-lisp
  (defun jrm/copy-file-path (arg)
    "Copy the current buffer's file path to the kill ring.

  Default: copies the absolute path.
  C-u: copies the path relative to the project.el root.
  C-u C-u: copies just the file name."
    (interactive "P")
               ...
               (cond
                ...
                ((equal arg '(4))
                 (if-let ((project (project-current)))
                   ...
                   (user-error "No project.el project found for this buffer")))
                ((equal arg '(16))
...
#+begin_src emacs-lisp
  ...
  (add-to-list
   'org-structure-template-alist
   '("g" . "src go :imports :exports both value"))
  (add-to-list
   'org-structure-template-alist
   '("b" . "src bash"))
  (add-to-list
...
#+begin_src emacs-lisp
  (setq project-kill-buffer-conditions '(buffer-file-name
   ...
   (and (derived-mode . comint-mode)
        (not (derived-mode . agent-shell-mode)))
